...
function setupMobileNavigation() {
  ...
  document.body.insertAdjacentHTML('beforeend', '<button class="mobile-nav-backdrop" type="button" aria-label="Close navigation"></button>');
  sidebar.insertAdjacentHTML('beforeend', '<button class="mobile-nav-close" type="button" aria-label="Close navigation"><span></span><span></span></button>');
  ...
  mobile.addEventListener?.('change', () => { if (!mobile.matches) close(); else sync(); });
  document.addEventListener('keydown', event => { if (event.key === 'Escape' && document.body.classList.contains('mobile-nav-open')) close(); });
  return true;
}
...
if (typeof document !== 'undefined') {
  const boot = () => {
    ...
    const observer = new MutationObserver(() => { if (setupMobileNavigation()) observer.disconnect(); });
