...
function readPreference() {
  try {
    return normalizeSidebarState(localStorage.getItem(storageKey));
  } catch {
    return 'expanded';
  }
}
...
function setupSidebarCollapse() {
  ...
  brand.insertAdjacentHTML(
    'afterend',
    '<button class="sidebar-collapse-toggle" type="button"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m14 6-6 6 6 6"/></svg><span>Collapse navigation</span></button>',
  );
...
if (typeof document !== 'undefined') {
  const boot = () => {
    ...
    const observer = new MutationObserver(() => {
      if (setupSidebarCollapse()) observer.disconnect();
    });
