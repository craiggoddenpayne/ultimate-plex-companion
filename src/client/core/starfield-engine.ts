export function initStarfield() {
  ...
  let width=0,height=0,dpr=1,stars:any[]=[],frame=0,lastPaint=0,running=true;
  ...
  function reset(star:any,initial=false){const origin=centre();star.z=initial?30+Math.random()*(depth-30):depth;const targetX=Math.random()*width,targetY=Math.random()*height;star.x=(targetX-origin.x)*star.z/focal;star.y=(targetY-origin.y)*star.z/focal;star.previousX=targetX;star.previousY=targetY;star.radius=.45+Math.random()*1.25;star.alpha=.52+Math.random()*.48;star.warm=Math.random()<.15;star.phase=Math.random()*Math.PI*2}
  function projected(star:any){const origin=centre(),scale=focal/Math.max(1,star.z);return {x:origin.x+star.x*scale,y:origin.y+star.y*scale}}

  function resize(){width=innerWidth;height=innerHeight;dpr=Math.min(devicePixelRatio||1,1.35);canvas.width=Math.round(width*dpr);canvas.height=Math.round(height*dpr);canvas.style.width=`${width}px`;canvas.style.height=`${height}px`;ctx.setTransform(dpr,0,0,dpr,0,0);const count=Math.min(380,Math.max(190,Math.round(width*height/5800)));stars=Array.from({length:count},()=>{const star:any={};reset(star,true);const point=projected(star);star.previousX=point.x;star.previousY=point.y;return star});paint(performance.now(),0)}
