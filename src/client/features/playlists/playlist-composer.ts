...
function previewMarkup(data) {
  if (!data.count)
    return '<div class="composer-empty"><b>No titles match this signal</b><span>Relax one or two filters and preview again.</span></div>';
...
export function renderPlaylistComposer(facets: any = {}, onCreated = () => {}) {
  ...
  form.addEventListener('change', () => {
    ...
    result.innerHTML =
      '<div class="composer-idle"><i></i><b>Signal changed</b><span>Preview again to validate the new criteria.</span></div>';
  });
  ...
  create.addEventListener('click', async () => {
    ...
    if (
      !window.confirm(
        `Are you sure you want to create “${title}” with up to ${Math.min(limit, preview.count)} matching items?`,
      )
    )
      return;
    create.disabled = true;
    ...
    try {
      ...
      create.textContent = 'Create in Plex';
    }
  });
}
