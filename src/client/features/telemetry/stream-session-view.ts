...
function codec(value) {
  const names = { h264: 'H.264', hevc: 'HEVC', h265: 'HEVC', eac3: 'E-AC-3', ac3: 'AC-3', aac: 'AAC', dca: 'DTS', truehd: 'TrueHD', opus: 'Opus', av1: 'AV1' };
  return names[String(value || '').toLowerCase()] || String(value || '').toUpperCase() || 'Unknown';
}

function decision(value) {
  return ({ directplay: 'Direct play', copy: 'Direct stream', transcode: 'Transcode', burn: 'Burned in', none: 'None' })[String(value || '').toLowerCase()] || value || 'Unknown';
}
...
function pictureDetail(item) {
  return [item.dimensions, item.videoProfile, item.videoBitDepth ? `${item.videoBitDepth}-bit` : '', item.videoFrameRate ? `${item.videoFrameRate} fps` : ''].filter(Boolean).join(' · ');
}

function audioDetail(item) {
  return [item.audioChannels ? `${item.audioChannels} channels` : '', item.audioLanguage, item.audioSampleRate ? `${Math.round(item.audioSampleRate / 1000)} kHz` : ''].filter(Boolean).join(' · ');
}
...
export function sessionMarkup(item) {
  const headline = [item.subtitle, item.resolution, codec(item.videoCodec), item.dynamicRange, codec(item.audioCodec), String(item.container || '').toUpperCase()].filter(Boolean).join(' · ');
  const networkDetail = [item.secure ? 'Secure' : 'Unsecured', item.address || item.publicAddress].filter(Boolean).join(' · ');
  const clientDetail = [item.product, item.platform, item.platformVersion, item.playerVersion ? `v${item.playerVersion}` : ''].filter(Boolean).join(' · ');
  const deliveryDetail = [decision(item.videoDecision), decision(item.audioDecision), item.protocol ? item.protocol.toUpperCase() : '', item.outputContainer ? item.outputContainer.toUpperCase() : ''].filter(Boolean).join(' · ');
  const subtitleValue = item.subtitleDisplayTitle || (item.subtitleCodec ? codec(item.subtitleCodec) : 'None selected');
  const subtitleDetail = item.subtitleCodec ? [item.subtitleLanguage, decision(item.subtitleDecision)].filter(Boolean).join(' · ') : 'No subtitle processing';
