...
function labShell() {
  return (
    ...
    tabNames
      ...
      .join('') +
    '</nav><div class="lab-stage" id="lab-stage"><div class="lab-loading"><span></span><h3>Mapping the edges</h3><p>Analysing relationships across your movie archive.</p></div></div>'
  );
}

function constellationView(data) {
  return (
    '<section class="lab-view constellation-view"><div class="view-copy"><span class="card-label">CULTURAL GRAPH</span><h2>Your library has a shape.</h2><p>Genres cluster around the filmmakers who appear most often in your collection. Hover over a signal to isolate its strongest relationships.</p><div class="graph-legend"><span><i></i>Genre</span><span><i></i>Director</span></div><div class="graph-top">' +
    data.graph.nodes
      .slice(0, 6)
      .map((node) => '<div><b>' + labEscape(node.label) + '</b><span>' + node.count + ' titles</span></div>')
      .join('') +
    '</div></div><div class="graph-wrap"><canvas id="culture-graph"></canvas><div id="graph-tooltip"></div></div></section>'
  );
}
...
function serendipityView(data) {
  const pair = data.doubleFeature;
  return (
    '<section class="lab-view serendipity-view"><div class="serendipity-copy"><span class="card-label">SERENDIPITY ENGINE · DAILY PAIRING</span><h2>Two films that should<br>never have met.</h2><p>Radar deliberately crosses distant genres to create an unexpected double feature from unwatched, highly rated films you already own.</p><div class="pair-equation"><span>CONTRAST</span><i>+</i><span>QUALITY</span><i>+</i><span>CURIOSITY</span></div></div><div class="double-feature">' +
    (pair.length === 2
      ? pair
          .map(
            (item, index) =>
...
function oracleView(data) {
  ...
  return (
    '<section class="lab-view oracle-view"><div class="oracle-core"><span class="oracle-r1"></span><span class="oracle-r2"></span><i>' +
    oracle.confidence +
    '%</i><b>SIGNAL<br>CONFIDENCE</b></div><div class="oracle-copy"><span class="card-label">VIEWING ORACLE</span><h2>Your habits leave<br>a gentle signature.</h2><p>This is pattern recognition, not prediction. It describes the rhythms already present in your Plex history.</p><div class="oracle-grid"><article><span>PEAK HOUR</span><b>' +
    time +
    '</b><small>' +
    oracle.peakHourPlays +
    ' sampled plays</small></article><article><span>PEAK DAY</span><b>' +
    days[oracle.peakDay] +
    '</b><small>' +
    oracle.peakDayPlays +
    ' sampled plays</small></article><article><span>STRONGEST GENRE</span><b>' +
    labEscape(oracle.favouriteGenre) +
    '</b><small>' +
    oracle.favouriteGenreCount +
    ' watched titles</small></article><article><span>EXPLORED</span><b>' +
    watchedPercent +
    '%</b><small>' +
    oracle.unwatchedMovies +
    ' films still waiting</small></article></div></div></section>'
  );
}

function drawGraph(graph) {
  const canvas = document.querySelector('#culture-graph');
  if (!canvas) return;
  ...
  function paint(hover = null) {
    ...
    for (const node of positioned) {
      const active =
        ...
        node.id === hover.id ||
        graph.edges.some(
          (e) => (e.source === hover.id && e.target === node.id) || (e.target === hover.id && e.source === node.id),
        );
      ctx.beginPath();
  ...
  canvas.onmousemove = (event) => {
    ...
    const hit = positioned.find((node) => Math.hypot(node.x - x, node.y - y) < node.r + 7);
    paint(hit);
    const tip = document.querySelector('#graph-tooltip');
    if (hit) {
      tip.textContent = hit.label + ' · ' + hit.count + ' titles';
  ...
  canvas.onmouseleave = () => {
    paint();
    document.querySelector('#graph-tooltip')?.classList.remove('visible');
  };
}
