...
export function connectionAdvice(message) {
  ...
  if (/rejected|401|unauthor/.test(text))
    return 'The server responded, but the token was refused. Copy only the value after X-Plex-Token= and try a newly generated token.';
  if (/timed out|8 seconds|timeout/.test(text))
    return 'The server address did not answer. Confirm port 32400 is reachable and that a firewall is not blocking the container.';
  if (/reach|refused|fetch failed|resolve|enotfound/.test(text))
    return 'Check the address from inside Docker. On Docker Desktop, try host.docker.internal; on a NAS or Linux host, use the Plex server’s LAN IP.';
  if (/json|404|not found/.test(text))
    return 'Use the Plex server base address only, such as http://192.168.1.25:32400. Remove /web and any library path.';
  if (/certificate|tls|ssl/.test(text))
    return 'Try the server’s HTTP LAN address on port 32400. Local self-signed HTTPS certificates may not validate inside the container.';
  return '';
}
...
function enhanceConnectionModal(modal) {
  ...
  pasteButton.addEventListener('click', async () => {
    try {
      ...
      pasteButton.textContent = 'Clipboard permission needed';
    }
  });
