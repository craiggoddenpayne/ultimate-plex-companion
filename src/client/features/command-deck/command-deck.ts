...
function updateDeck(data) {
  ...
  dashboard.querySelector('.hero-row h1').innerHTML = data.sessions.length
    ? 'Your universe is<br><em>alive right now.</em>'
    : 'Your universe is<br><em>running quietly.</em>';
  dashboard.querySelector('.hero-row p').textContent = data.sessions.length
    ? data.sessions.length +
      ' active ' +
      (data.sessions.length === 1 ? 'stream' : 'streams') +
      ' across your Plex household.'
    : 'Everything is connected. Nothing is streaming right now.';
  ...
  healthCard.querySelector('p').textContent =
    data.health.latencyMs +
    ' ms command response · ' +
    data.health.transcodes +
    ' active ' +
    (data.health.transcodes === 1 ? 'transcode' : 'transcodes');
  ...
  const streamCount = dashboard.querySelector('#stream-count'),
    streamSummary = dashboard.querySelector('#stream-summary');
  streamCount.textContent = data.sessions.length;
  streamSummary.textContent = data.sessions.length
    ? data.sessions.length - data.health.transcodes + ' direct · ' + data.health.transcodes + ' transcodes'
    : 'No active sessions';
  ...
  dashboard.querySelector('#library-summary').textContent =
  ...
  streamBars.innerHTML = (data.sessions.length ? data.sessions : Array.from({ length: 8 }, () => ({ progress: 4 })))
    .slice(0, 8)
    .map((session) => '<i style="height:' + Math.max(8, session.progress || 4) + '%"></i>')
    .join('');
  ...
  bars.innerHTML = data.watch.daily
    .map(
      (day) =>
        '<i title="' +
        day.date +
        ...
        day.minutes +
        ' min" style="height:' +
        Math.max(8, Math.round((day.minutes / peak) * 100)) +
        '%"></i>',
    )
    .join('');
    ...
    ? data.activity
        ...
        .map(
          (item) =>
            ...
            activityVisual(item) +
            '<p><b>' +
            deckEscape(item.title) +
            '</b><small>' +
            deckEscape(item.detail) +
            '</small></p><time>' +
            timeAgo(item.at) +
            '</time></div>',
        )
        .join('')
    : '<div class="deck-empty">No recent activity from Plex.</div>';
...
async function loadCommandDeck() {
  ...
  try {
    ...
    if (sync) {
      sync.innerHTML = '<i class="offline"></i>Deck offline';
      sync.title = error.message;
    }
  } finally {
    if (sync) sync.classList.remove('refreshing');
  }
}
...
loadCommandDeck();
setInterval(() => {
  if (!document.hidden && location.hash !== '#library' && location.hash !== '#radar') loadCommandDeck();
}, 60000);
