...
function historyIdentity(item) {
  if (item.type === 'episode')
    return [item.librarySectionID, 'episode', item.grandparentTitle, item.parentIndex, item.index].join('|');
...
function recentHistory(records, catalog) {
  ...
  for (const item of catalog) {
    index.set(historyIdentity({ ...item, librarySectionID: item.librarySectionID || item.libraryKey }), item);
    if (item.ratingKey) byRatingKey.set(String(item.ratingKey), item);
  }
  return records
    .map((record) => ({
      record,
      media: byRatingKey.get(String(record.ratingKey || '')) || index.get(historyIdentity(record)),
    }))
...
export async function commandDeck(config, dependencies) {
  ...
  const timedOverview = (async () => {
    ...
    const result = await overview(config);
    plexLatency = Date.now() - signal;
    return result;
  })();
  const [base, historyData, recentData, catalog] = await Promise.all([
    timedOverview,
    plexFetch(config, historyPath),
  ...
  const recentAdded = (recentData.MediaContainer?.Metadata || []).map((item) => ({
    type: 'added',
    title: item.title || item.grandparentTitle || 'New item',
    detail: `Added to ${item.librarySectionTitle || 'Plex'}`,
    at: Number(item.addedAt || 0),
  ...
  const recentWatched = matched.slice(0, 12).map(({ record, media }) => ({
    type: 'watched',
    title: record.grandparentTitle || record.title || media.title,
    detail:
      record.type === 'episode'
        ? `Watched S${String(record.parentIndex || 0).padStart(2, '0')} E${String(record.index || 0).padStart(2, '0')}`
        : 'Watched',
    at: Number(record.viewedAt || 0),
    poster:
      record.ratingKey || media.ratingKey
        ? `/api/art/${encodeURIComponent(record.ratingKey || media.ratingKey)}`
        : null,
  }));
  const live = base.sessions.map((session) => ({
    type: 'stream',
    title: session.title,
    detail: `${session.user} · ${session.mode}`,
    at: Math.floor(Date.now() / 1000),
    poster: session.poster || (session.ratingKey ? `/api/art/${encodeURIComponent(session.ratingKey)}` : null),
  }));
  ...
  return {
    ...
    health: {
      score: healthScore,
      latencyMs: latency,
      status: healthScore >= 94 ? 'Optimal' : healthScore >= 85 ? 'Healthy' : 'Under load',
      transcodes,
    },
    watch: intelligence.watch,
    taste: intelligence.taste,
    activity,
