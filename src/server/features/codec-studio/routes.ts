  ...
  return async (context) => {
    ...
    if (pathname === '/api/optimization/config' && req.method === 'GET') {
      ...
      json(res, 200, {
        ...
        encoderAvailable,
        targets,
        managed: Boolean(process.env.PLEX_MEDIA_ROOT || process.env.MEDIA_ROOT),
      });
      return true;
    }
    if (pathname === '/api/optimization/config' && req.method === 'POST') {
      ...
      if (process.env.PLEX_MEDIA_ROOT || process.env.MEDIA_ROOT) {
        json(res, 409, { error: 'Media paths are managed by environment variables.' });
        return true;
      }
      const settings = context.optimizationSettings({ optimization: await context.body(req) });
      if (!isAbsolute(settings.plexPathRoot) || !isAbsolute(settings.mediaPathRoot))
        throw new Error('Both media roots must be absolute paths.');
    ...
    if (pathname === '/api/optimization/jobs' && req.method === 'GET') {
      json(res, 200, {
        jobs: [...queue.jobs.values()].map(queue.publicJob),
        paused: queue.isPaused(),
        summary: optimizationSummary(queue.jobs, queue.activeJob()),
      });
      return true;
    }
    ...
    if (replace && req.method === 'POST') {
      ...
      if (!job) {
        json(res, 404, { error: 'Optimization job not found.' });
        return true;
      }
