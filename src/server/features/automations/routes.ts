export function createAutomationRoutes(automationEngine) {
  return async context => {
    ...
    if (pathname === '/api/automations' && req.method === 'GET') { json(res, 200, await automationEngine.list()); return true; }
    if (pathname === '/api/automations' && req.method === 'POST') { json(res, 201, { rule:await automationEngine.create(await context.body(req)) }); return true; }
    if (pathname === '/api/automations/state' && req.method === 'PATCH') { json(res, 200, { paused:await automationEngine.setPaused((await context.body(req)).paused) }); return true; }

    const parts = pathname.split('/');
    if (parts.length === 5 && parts[1] === 'api' && parts[2] === 'automations' && parts[4] === 'run' && req.method === 'POST') {
