...
function selectedStream(part, streamType) {
  ...
  return streams.find((stream) => Number(stream.streamType) === streamType && plexBoolean(stream.selected))
    || streams.find((stream) => Number(stream.streamType) === streamType)
    || {};
}

function dynamicRange(video) {
  const description = [video.dynamicRange, video.displayTitle, video.extendedDisplayTitle, video.colorTrc].filter(Boolean).join(' ');
...
export function sessionDetail(item, index = 0) {
  ...
  const subtitleDecision = transcode?.subtitleDecision || media.subtitleDecision || (subtitle.id ? 'directplay' : 'none');
  const requiresEncode = videoDecision === 'transcode' || audioDecision === 'transcode' || subtitleDecision === 'burn';
  const mode = requiresEncode ? 'Transcoding' : transcode || videoDecision === 'copy' || audioDecision === 'copy' ? 'Direct Stream' : 'Direct Play';
...
export async function streamTelemetry(config, plexFetch) {
  ...
  const recent = await Promise.all(recentRecords.map(async (record) => {
...
export async function peopleTelemetry(config, plexFetch, requestedDays = 90) {
  ...
  const people = relevantAccounts.map((account, index) => {
    ...
    return { id: account.id, name: account.name || `User ${index + 1}`, initials: String(account.name || 'U').split(/\s+/).map((value) => value[0]).join('').slice(0, 2).toUpperCase(), plays: plays.length, movies, episodes, share: Math.round(plays.length / totalPlays * 100), lastSeen: latest, active: Boolean(current), nowPlaying: current?.title || null, tone: ['amber', 'violet', 'cyan', 'rose'][index % 4] };
