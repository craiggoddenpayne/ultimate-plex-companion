...
function matchHistory(items, history) {
  ...
  for (const item of items) {
    for (const title of [item.title, item.grandparentTitle])
      if (title && !byTitle.has(normalise(title))) byTitle.set(normalise(title), item);
  }
  return history
    .map((record) => {
      const media =
        byKey.get(String(record.ratingKey || '')) || byTitle.get(normalise(record.grandparentTitle || record.title));
      return media ? { record, media } : null;
    })
    .filter(Boolean)
    .sort((a, b) => Number(b.record.viewedAt || 0) - Number(a.record.viewedAt || 0));
}
...
export function buildMemoryLane(items, history, nowSeconds = Math.floor(Date.now() / 1000)) {
  ...
  return {
    matchedPlays: matches.length,
    uniqueTitles: new Set(matches.map(({ media }) => String(media.ratingKey || normalise(media.title)))).size,
    favouriteDecade: favourite ? `${favourite[0]}s` : 'Still forming',
    favouriteDecadePlays: favourite?.[1] || 0,
    months: calendarMonths(history, nowSeconds),
    recent,
  };
}
...
function moodForecast(genre, playCount) {
  if (!playCount)
    return {
      name: 'Quiet orbit',
      detail: 'No recent viewing pressure. The atmosphere is open for something new.',
      tone: 'quiet',
    };
  if (/horror|thriller|crime|mystery/i.test(genre))
    return {
      name: 'Electric storm',
      detail: 'Tension and mystery are dominating your recent viewing climate.',
      tone: 'storm',
    };
  if (/comedy|animation|family|romance/i.test(genre))
    return {
      name: 'Warm front',
      detail: 'Lighter, warmer stories are moving through your recent history.',
      tone: 'warm',
    };
  if (/sci|fantasy|adventure/i.test(genre))
    return {
      name: 'Cosmic pressure',
      detail: 'Expansive worlds and speculative stories are pulling the strongest signal.',
      tone: 'cosmic',
    };
  if (/documentary|history|biography/i.test(genre))
    return {
      name: 'Clear focus',
      detail: 'Curiosity and real-world stories are defining the current atmosphere.',
      tone: 'clear',
    };
  return {
    name: 'Mixed skies',
    detail: `${genre} currently leads an otherwise varied viewing climate.`,
    tone: 'mixed',
  };
}
...
export function buildMoodWeather(items, history, nowSeconds = Math.floor(Date.now() / 1000)) {
  ...
  const recentGenres = countGenres(recent),
    previousGenres = countGenres(previous);
  const genreNames = new Set([...recentGenres.keys(), ...previousGenres.keys()]);
  const signals = [...genreNames]
    .map((genre) => ({
      genre,
      recent: recentGenres.get(genre) || 0,
      previous: previousGenres.get(genre) || 0,
  ...
  const weekends = recentRecords.filter((record) =>
    [0, 6].includes(new Date(Number(record.viewedAt || 0) * 1000).getDay()),
  ).length;
  return {
    periodDays: 30,
    recentPlays: recent.length,
    previousPlays: previous.length,
    momentum: recent.length - previous.length,
    ...
    forecast: { ...moodForecast(lead, recent.length), leadGenre: lead },
    signals,
  };
}
...
export function buildRuntimeWormhole(items) {
  ...
  return {
    ...
    unwatchedHours: Math.round(unwatched.reduce((sum, item) => sum + minutes(item), 0) / 60),
    medianMinutes,
    buckets,
...
export function buildArchiveAnomalies(items, nowSeconds = Math.floor(Date.now() / 1000)) {
  ...
  for (const item of items) {
    ...
    for (const director of tags(item, 'Director'))
      directorCounts.set(director, (directorCounts.get(director) || 0) + 1);
  }
  const unwatched = items.filter((item) => !Number(item.viewCount || 0));
  const buriedGems = unwatched
    .filter((item) => rating(item) >= 7.5)
    .map((item) => ({
      item,
      daysWaiting: item.addedAt ? Math.max(0, Math.floor((nowSeconds - Number(item.addedAt)) / DAY)) : null,
    }))
    .sort((a, b) => rating(b.item) - rating(a.item) || (b.daysWaiting || 0) - (a.daysWaiting || 0))
    .slice(0, 6)
    .map(({ item, daysWaiting }) => publicItem(item, { daysWaiting }));
  const oldestUnwatched = [...unwatched]
    .filter((item) => Number(item.year))
    .sort((a, b) => Number(a.year) - Number(b.year))
    .slice(0, 5)
    .map((item) => publicItem(item));
  return {
    rareGenres: [...genreCounts.entries()]
      .filter(([, count]) => count <= 2)
      .sort((a, b) => a[1] - b[1] || a[0].localeCompare(b[0]))
      .slice(0, 10)
      .map(([genre, count]) => ({ genre, count })),
    oneOffDirectors: [...directorCounts.entries()]
