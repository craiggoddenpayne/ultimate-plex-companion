export function createConnectionRoutes() {
  return async context => {
    ...
    if (pathname === '/api/config' && req.method === 'GET') {
      ...
      json(res, 200, { configured:Boolean(config), plexUrl:config?.plexUrl || '', tokenSource:context.envConfig ? 'environment' : config ? 'saved' : 'none' });
      return true;
    }
    ...
    if (pathname === '/api/config' && req.method === 'POST') {
      if (context.envConfig) { json(res, 409, { error:'Settings are managed by PLEX_URL and PLEX_TOKEN environment variables.' }); return true; }
      const previous = await context.savedConfig();
      const config = { ...context.normalizeConfig(await context.body(req)), optimization:previous?.optimization || context.optimizationSettings() };
