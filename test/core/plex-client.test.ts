...
test('Plex configuration validation strips trailing slashes and rejects embedded credentials', () => {
  ...
  assert.throws(() => normalizePlexConfig({ plexUrl:'http://user:pass@plex.local', token:'token' }), /Do not include credentials/);
});
...
test('Plex client injects identity and keeps the token at the transport boundary', async () => {
  ...
  const client = createPlexClient(async (url, options) => {
    request = { url, options };
    return new Response(JSON.stringify({ MediaContainer:{ size:1 } }), { status:200, headers:{ 'content-type':'application/json' } });
  });
