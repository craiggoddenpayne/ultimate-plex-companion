...
test('System Memory returns artwork for live, added and watched activity', async () => {
  ...
  const plexFetch = async (_config, path) => {
    ...
    if (path.startsWith('/status/sessions/history')) return { MediaContainer:{ Metadata:[{ ratingKey:'22', type:'movie', title:'Watched Film', viewedAt:now - 60 }] } };
    if (path.startsWith('/library/recentlyAdded')) return { MediaContainer:{ Metadata:[{ ratingKey:'11', title:'New Film', addedAt:now - 120, librarySectionTitle:'Films' }] } };
    throw new Error(`Unexpected Plex path: ${path}`);
  };
  const result = await commandDeck({}, {
    plexFetch,
    overview:async () => ({ sessions:[{ ratingKey:'33', title:'Live Film', user:'Craig', mode:'Direct Play' }], libraryCount:1, titleCount:3, server:{} }),
    libraryItems:async () => [{ ratingKey:'22', librarySectionID:'1', type:'movie', title:'Watched Film', duration:7_200_000, Genre:[{ tag:'Drama' }] }],
  });
