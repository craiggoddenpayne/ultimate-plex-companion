...
test('Command Deck derives weighted taste and weekly comparison', () => {
  ...
  const entry = (days, title, genres, type = 'movie', year = 2010) => ({
    record: { viewedAt: now - days * 86400, type, title },
    media: { title, type, year, duration: 7_200_000, Genre: genres.map((tag) => ({ tag })) },
  });
  const matches = [
    entry(1, 'Arrival', ['Science Fiction', 'Drama'], 'movie', 2016),
    entry(2, 'Dune', ['Science Fiction', 'Adventure'], 'movie', 2021),
    entry(9, 'Old Week', ['Comedy']),
  ];
