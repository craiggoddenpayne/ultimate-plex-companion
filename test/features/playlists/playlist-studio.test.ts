...
import {
  buildPlaylistGenerators,
  composePlaylist,
  createGeneratedPlaylist,
  normalizeComposerCriteria,
  playlistComposerFacets,
  playlistCreatePath,
} from '../../../src/server/features/playlists/playlist-studio-server.ts';
...
test('playlist generators produce useful live criteria', () => {
  ...
  const items = [
    movie(1, 'New', { addedAt: now - 100, Genre: [{ tag: 'Science Fiction' }] }),
    movie(2, 'Short'),
    movie(3, 'Epic', { duration: 170 * 60_000, Media: [{ videoResolution: '4k' }] }),
    movie(4, 'Partial', { viewOffset: 50 * 60_000 }),
  ];
...
test('playlist creation builds a confirmed Plex video playlist', async () => {
  const calls = [];
  const dependencies = {
    plexFetch: async () => ({ MediaContainer: { Directory: [{ key: '1', type: 'movie' }] } }),
    libraryItems: async () => [movie(8, 'Primer')],
    inspectPlex: async () => ({ machineIdentifier: 'machine' }),
    plexCommand: async (...args) => calls.push(args),
  };
  const created = await createGeneratedPlaylist({}, dependencies, {
    generatorId: 'essentials',
    title: 'Great Films',
    limit: 20,
    confirmed: true,
  });
  ...
  assert.match(
    decodeURIComponent(calls[0][1]),
    /server:\/\/machine\/com\.plexapp\.plugins\.library\/library\/metadata\/8/,
  );
...
test('custom composer combines filters and ranks matching Plex items', () => {
  ...
  const result = composePlaylist(items, {
    type: 'movie',
  ...
  assert.deepEqual(
    result.items.map((item) => item.ratingKey),
    ['30'],
  );
  ...
  const normalized = normalizeComposerCriteria({
    type: 'music',
    watchState: 'maybe',
    minRating: 99,
    maxMinutes: 900,
...
test('custom composer exposes useful facets and creates only confirmed matches', async () => {
  const items = [
    movie(40, 'Drama', { year: 1995, Genre: [{ tag: 'Drama' }] }),
    movie(41, 'Comedy', { year: 2004, Genre: [{ tag: 'Comedy' }] }),
  ];
  const facets = playlistComposerFacets(items);
  assert.deepEqual(facets.decades, [2000, 1990]);
  assert.deepEqual(
    facets.genres.map((item) => item.value),
    ['Comedy', 'Drama'],
  );
  const calls = [];
  const dependencies = {
