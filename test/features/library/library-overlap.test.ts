...
test('overlaps expose detailed, selectable media copies', () => {
  const first = {
    ...
    libraryTitle: 'Movies',
    libraryKey: '1',
    facts: { size: 1000, versions: [{ media: media(101, '/media/Arrival/Arrival-4K.mkv', '4k', 1000), size: 1000 }] },
  };
  const second = {
    ratingKey: '12',
    type: 'movie',
    ...
    facts: {
      size: 500,
      versions: [{ media: media(102, '/archive/Arrival/Arrival-1080p.mkv', '1080', 500), size: 500 }],
    },
  };
...
test('overlap deletion requires a current overlap and explicit confirmation', async () => {
  ...
  const report = {
    editions: {
      duplicates: [{ title: 'Arrival', copies: [copy, { ratingKey: '12', mediaId: '102' }] }],
      versioned: [],
    },
  };
  const calls = [];
  const dependencies = {
    plexFetch: async () => ({
      MediaContainer: {
        Metadata: [{ title: 'Arrival', Media: [media(101, '/media/Arrival/Arrival-4K.mkv', '4k', 1000)] }],
      },
    }),
    plexDelete: async (_config, path) => calls.push(path),
    invalidate: () => calls.push('invalidated'),
  };
  await assert.rejects(
    () => deleteOverlap({}, dependencies, { ratingKey: '11', mediaId: '101', confirmed: false }, report),
    /Confirm deletion/,
  );
