  ...
  const plexFetch = async () => ({
    MediaContainer: {
      Hub: [
        {
          title: 'Movies',
          Metadata: [{ ratingKey: '1', title: 'Arrival', type: 'movie', year: 2016, duration: 6960000 }],
        },
        {
          title: 'Search',
          Metadata: [
...
test('Companion answers from live dependencies and notifications expose actionable signals', async () => {
  const dependencies = {
    ...
    storageAnalysis: async () => ({
      estimatedSaving: 1024 ** 3,
      candidateCount: 2,
      scanned: 42,
      totalSize: 5 * 1024 ** 3,
      averageConfidence: 91,
    }),
    discoveryRecommendations: async () => ({ results: [] }),
    streamTelemetry: async () => ({
      summary: { active: 0, direct: 0, transcodes: 0, totalBandwidth: 0 },
      sessions: [],
    }),
    plexFetch: async () => ({ MediaContainer: { Metadata: [] } }),
    automationEngine: {
      list: async () => ({ paused: false, rules: [{ enabled: true }], runs: [], templates: Array(6) }),
    },
  };
  ...
  const notices = await companionNotifications(
    {},
    {
      ...dependencies,
      getJobs: () => [{ id: 'job', state: 'ready', title: 'Film', saving: 1024, updatedAt: new Date().toISOString() }],
    },
  );
