...
test('saves Plex credentials securely and returns a live overview', async (t) => {
  ...
  const mock = createServer((req, res) => {
    ...
    const payload =
      req.url === '/'
        ? { MediaContainer: { friendlyName: 'Test Plex' } }
        : req.url === '/identity'
          ? { MediaContainer: { version: '1.42.2', machineIdentifier: 'abc' } }
          : req.url === '/library/sections'
            ? { MediaContainer: { Directory: [{ key: '1', title: 'Movies', type: 'movie' }] } }
            : req.url === '/status/sessions'
              ? {
                  MediaContainer: {
                    Metadata: [
                      {
                        ...
                        User: [{ title: 'Craig' }],
                        Player: [{ title: 'TV' }],
                        Media: [{ videoResolution: '4k', videoDecision: 'directplay', Part: [{ size: 100 }] }],
                      },
                    ],
                  },
                }
              : req.url.startsWith('/library/metadata/99')
                ? {
                    MediaContainer: {
                      Metadata: [
                        {
                          ...
                          Media: [
                            {
                              ...
                              videoCodec: 'h264',
                              bitrate: 30000,
                              Part: [{ size: 10 * 1024 ** 3, file: sourcePath }],
                            },
                          ],
                        },
                      ],
                ...
                : req.url.startsWith('/library/sections/1/all')
                  ? {
                      MediaContainer: {
                        totalSize: 1,
                        Metadata: [
  ...
  const app = spawn(process.execPath, ['server.js'], {
    cwd: process.cwd(),
    env: { ...process.env, PORT: String(appPort), CONFIG_DIR: configDir },
    stdio: 'ignore',
  });
  t.after(async () => {
  ...
  assert.equal(
    plexLink.headers.get('location'),
    'https://app.plex.tv/desktop/#!/server/abc/details?key=%2Flibrary%2Fmetadata%2F99',
  );
  ...
  assert.equal(analysis.estimatedSaving, Math.round(10 * 1024 ** 3 * 0.35));

  const encoderConfig = await fetch(`http://127.0.0.1:${appPort}/api/optimization/config`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ plexPathRoot: configDir, mediaPathRoot: configDir, crf: 20, preset: 'fast' }),
  });
  assert.equal(encoderConfig.status, 200);
  const queued = await (
    await fetch(`http://127.0.0.1:${appPort}/api/optimization/jobs`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ ratingKey: '99' }),
    })
  ).json();
  let job = queued.job;
  ...
  const replaced = await fetch(`http://127.0.0.1:${appPort}/api/optimization/jobs/${job.id}/replace`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
