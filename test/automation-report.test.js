...
import { arrivalReport } from '../automation-report-server.js';
...
test('arrival digest retains and renders every Plex media path', () => {
  const result = arrivalReport([{ ratingKey:'42', title:'Arrival', year:2016, type:'movie', addedAt:1_700_000_000, Media:[
  ...
  const html = renderAutomationReports([{ id:'run-1', ruleName:'Arrival Digest', status:'success', result }], { escape, relativeTime:()=> 'now', bytes:String });
