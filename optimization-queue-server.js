...
export function optimizationEta(job, now = Date.now()) {
  if (job?.state !== 'encoding' || !job.startedAt || Number(job.progress) <= 0 || Number(job.progress) >= 100) return null;
...
export function optimizationSummary(jobs, activeJob = null) {
  ...
  const counts = Object.fromEntries(['queued','preparing','encoding','verifying','ready','failed','replaced','cancelled'].map(state => [state, 0]));
  const targets = {};
  ...
  return { total:list.length, active:list.filter(job => activeStates.has(job.state)).length, completed:counts.replaced + counts.cancelled, counts, estimatedSaving, reclaimed, targets, activeJob };
}

export function reorderQueuedJob(jobs, id, direction) {
  const ordered = [...jobs.values()];
  const queuedIndexes = ordered.map((job, index) => job.state === 'queued' ? index : -1).filter(index => index >= 0);
