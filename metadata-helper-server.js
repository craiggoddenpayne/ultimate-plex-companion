...
export function metadataUpdate(item,input={}){
  ...
  if(!/^\d+$/.test(ratingKey)||!/^\d+$/.test(libraryKey))throw new Error('Plex metadata identifiers are unavailable for this item.');
  ...
  for(const field of textFields){
    ...
    if(field==='originallyAvailableAt'&&value&&!validDate(value))throw new Error('Release date must use YYYY-MM-DD.');
