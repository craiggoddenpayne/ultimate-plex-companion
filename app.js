import { initStarfield } from './starfield-engine.js';
import { featureSets, navigation as nav, pageCopy } from './feature-registry.js';
const icons = {
  ...
  spark: '<path d="m12 3 1.3 4.7L18 9l-4.7 1.3L12 15l-1.3-4.7L6 9l4.7-1.3zM19 16l.6 2.4L22 19l-2.4.6L19 22l-.6-2.4L16 19l2.4-.6z"/>',
...
const formatBytes = bytes => {
  ...
  const units = ['B','KB','MB','GB','TB']; let value = Number(bytes), unit = 0;
  while (value >= 1024 && unit < units.length - 1) { value /= 1024; unit++; }
  return `${value.toFixed(unit > 2 ? 1 : 0)} ${units[unit]}`;
};

const streams = [
  { title: 'Blade Runner 2049', meta: '4K HDR · Direct Play', user: 'Alex', device: 'Living Room TV', progress: 68, tone: 'amber' },
  { title: 'Severance', meta: 'S02 E04 · 1080p', user: 'Sam', device: 'Apple TV', progress: 41, tone: 'violet' },
  { title: 'Dune: Part Two', meta: '4K · Transcoding', user: 'Taylor', device: 'Chrome', progress: 23, tone: 'cyan' },
];
...
async function openSettings() {
  ...
  modal.querySelectorAll('[data-action="close"]').forEach(button => button.addEventListener('click', () => modal.remove()));
  ...
  const run = async (save) => {
    result.className = 'connection-result loading'; result.textContent = save ? 'Saving and connecting…' : 'Testing connection…';
    try {
      const response = await request(save ? '/api/config' : '/api/config/test', { method:'POST', body:JSON.stringify(credentials()) });
      result.className = 'connection-result success'; result.innerHTML = `<i></i> Connected to ${escapeHtml(response.server.name)} · Plex ${escapeHtml(response.server.version)}`;
      if (save) { setTimeout(() => modal.remove(), 650); await loadPlexOverview(); }
    } catch (error) { result.className = 'connection-result error'; result.textContent = error.message; }
  };
...
async function loadPlexOverview() {
  ...
  try {
    ...
    dot.className = 'status-dot'; serverName.textContent = data.server.name; serverState.textContent = 'ONLINE';
    ...
    document.querySelector('#library-summary').textContent = `Titles across ${data.libraryCount} ${data.libraryCount === 1 ? 'library' : 'libraries'}`;
...
async function loadStorageSignal() {
  try {
    ...
    if (!title) return;
    title.innerHTML = data.candidateCount ? `Your library may reclaim<br><em>${formatBytes(data.estimatedSaving)}</em> at transparent quality.` : 'Your library is already<br><em>beautifully lean.</em>';
    document.querySelector('#storage-suggestion-body').textContent = data.candidateCount ? `${data.candidateCount} legacy-codec files are worth reviewing. Nothing will be changed automatically.` : `No files crossed the conservative optimization threshold.`;
    document.querySelector('#storage-confidence').textContent = data.candidateCount ? `${data.averageConfidence}%` : 'HIGH';
    document.querySelector('#storage-saving').textContent = formatBytes(data.estimatedSaving);
  } catch {
    document.querySelector('#storage-suggestion-title').innerHTML = 'Connect Plex to unlock<br><em>library intelligence.</em>';
    document.querySelector('#storage-suggestion-body').textContent = 'Companion uses read-only media metadata to find opportunities safely.';
  }
}
...
async function runStorageScan(force = true) {
  ...
  button.disabled = true; button.innerHTML = `${icon('radar')} Reading Plex metadata…`;
  results.innerHTML = '<div class="scan-empty scanning"><span class="scan-orbit"></span><h3>Mapping your library</h3><p>Large libraries can take a moment. Results are cached for ten minutes.</p></div>';
  try {
    ...
    document.querySelector('#scan-libraries').textContent = `Across ${data.libraries} video ${data.libraries === 1 ? 'library' : 'libraries'}`;
    ...
    results.innerHTML = data.candidates.length ? `<div class="candidate-table">
      ...
      </div>` : '<div class="scan-empty success"><span>✓</span><h3>Your library is already lean</h3><p>No supported legacy-codec files crossed the conservative review threshold.</p></div>';
    results.querySelectorAll('.encode-button').forEach((button,index) => button.addEventListener('click', () => startOptimization(button.dataset.ratingKey, button.dataset.title, data.candidates[index].codec)));
  ...
  } finally { button.disabled = false; button.innerHTML = `${icon('radar')} Scan again`; }
}

let jobPollTimer;

function modernizerModal(title,sourceCodec){return '<div class="modal-wrap" id="modernizer-modal"><div class="modal-backdrop" data-modernizer-close></div><section class="settings-modal modernizer-modal"><button class="modal-close" data-modernizer-close>×</button><span class="eyebrow">CODEC MODERNIZER · VERIFIED CONVERSION</span><h2>Choose a modern format</h2><p>Convert <b>'+escapeHtml(title)+'</b> from '+escapeHtml(sourceCodec)+' while copying its audio, subtitles, chapters and metadata.</p><div class="modernizer-source">ORIGINAL PRESERVED UNTIL YOU APPROVE REPLACEMENT</div><div class="modernizer-options"><label class="modernizer-option"><input type="radio" name="target-codec" value="hevc" checked><b>HEVC / H.265</b><small>Best compatibility across modern Plex clients with strong space savings.</small><em>RECOMMENDED</em></label><label class="modernizer-option"><input type="radio" name="target-codec" value="av1"><b>AV1</b><small>Higher compression efficiency, but slower encoding and newer client support.</small><em>EXPERIMENTAL</em></label></div><div class="settings-actions"><button class="test-button" data-modernizer-close>Cancel</button><button class="primary-button" id="stage-modernization">Stage conversion</button></div></section></div>'}

function startOptimization(ratingKey,title,sourceCodec){document.querySelector('#modernizer-modal')?.remove();document.body.insertAdjacentHTML('beforeend',modernizerModal(title,sourceCodec));const modal=document.querySelector('#modernizer-modal'),close=()=>modal.remove(),button=modal.querySelector('#stage-modernization');modal.querySelectorAll('[data-modernizer-close]').forEach(item=>item.onclick=close);request('/api/optimization/config').then(config=>{for(const target of config.targets||[]){const input=modal.querySelector('[value="'+target.key+'"]');if(input){input.disabled=!target.available;input.closest('.modernizer-option').classList.toggle('unavailable',!target.available)}}const available=modal.querySelector('[name="target-codec"]:not(:disabled)');if(modal.querySelector('[name="target-codec"]:checked')?.disabled&&available)available.checked=true;if(!available){button.disabled=true;button.textContent='No modern encoder available'}}).catch(()=>{});button.onclick=async()=>{const targetCodec=modal.querySelector('[name="target-codec"]:checked').value,targetLabel=targetCodec==='av1'?'AV1':'HEVC';button.disabled=true;button.textContent='Checking encoder…';try{await request('/api/optimization/jobs',{method:'POST',body:JSON.stringify({ratingKey,targetCodec})});close();toast(title+' added to the '+targetLabel+' conversion queue');pollOptimizationJobs()}catch(error){button.disabled=false;button.textContent='Stage conversion';toast(error.message)}}}
...
function openReplacement(job) {
  ...
  const modal = document.querySelector('#replace-modal'); const button = modal.querySelector('#confirm-replace');
  modal.querySelectorAll('[data-action="close"]').forEach(el => el.addEventListener('click', () => modal.remove()));
  button.addEventListener('click', async () => {
    button.disabled = true; button.textContent = 'Replacing…';
    try { await request(`/api/optimization/jobs/${job.id}/replace`, { method:'POST', body:JSON.stringify({ confirmed:true }) }); modal.remove(); toast(`${job.title}: original replaced · ${formatBytes(job.saving)} reclaimed`); pollOptimizationJobs(); runStorageScan(true); }
...
function render() {
  document.querySelector('#app').innerHTML = `<canvas id="starfield" aria-hidden="true"></canvas><div class="cosmic-haze" aria-hidden="true"></div><div class="app-shell">${sidebar()}<main>${header()}<div class="page-wrap">${dashboard()}${libraryPage()}${Object.keys(pageCopy).filter(id => id !== 'library').map(genericPage).join('')}</div></main></div>`;
  bind();
  initStarfield();
  loadPlexOverview();
...
function route(id) {
  ...
  if (target === 'library' && document.querySelector('#scan-items')?.textContent === '—') setTimeout(() => runStorageScan(false), 120);
}

function toast(message) {
  const el = document.createElement('div'); el.className = 'toast'; el.innerHTML = `${icon('spark')}<span>${message}</span>`;
  document.querySelector('#toast-region').append(el); setTimeout(() => el.remove(), 3300);
}

function bind() {
  document.querySelectorAll('[data-nav], [data-route]').forEach(el => el.addEventListener('click', e => { e.preventDefault(); route(el.dataset.nav || el.dataset.route); }));
  document.querySelector('.mobile-brand').addEventListener('click', () => document.querySelector('.sidebar').classList.toggle('open'));
  document.querySelectorAll('[data-action]').forEach(el => el.addEventListener('click', () => {
    ...
    else if (action === 'send') { toast('Companion query queued — Plex connection comes next.'); document.querySelector('#assistant-modal')?.remove(); }
    else if (action === 'review') { route('library'); setTimeout(() => runStorageScan(false), 250); }
    else toast(action === 'notifications' ? 'You’re all caught up.' : 'Interactive prototype opened.');
  }));
  document.querySelectorAll('.prompt-chips button').forEach(el => el.addEventListener('click', () => { const input = document.querySelector('.ask-input input'); if (input) input.value = el.textContent; }));
  document.querySelector('#global-search').addEventListener('keydown', e => { if (e.key === 'Enter' && e.target.value) toast(`Searching for “${e.target.value}”`); });
